...
There are 6 control parameters:
    ...
    mu      - The fraction of the convective flux carried by the adiabatic gradient at z = 0 (below the heating layer)
    Lz      - The height of the box
    aspect  - The aspect ratio (Lx = aspect * Lz)

Usage:
    erf_step_AN_3D.py [options] 
    erf_step_AN_3D.py <config> [options] 
...
Options:
    ...
    --mu=<frac>              Fbot = mu * F_conv [default: 1e-3]
...
def run_cartesian_instability(args):
    ...
    data_dir += "_Re{}_P{}_mu{}_S{}_Lz{}_Lcz{}_Pr{}_a{}_Titer{}_{}x{}x{}".format(args['--Re'], args['--P'], args['--mu'], args['--S'], args['--Lz'], args['--L_cz'], args['--Pr'], args['--aspect'], args['--T_iters'], args['--nx'], args['--ny'], args['--nz'])
    ...
    #Model values
    k_rz = dH / (P * S) 
    k_cz = k_rz * ( mu / (1 + mu + invP) )
    k_ad = k_rz * ( (1 + mu) / (1 + mu + invP) )
    delta_k = k_rz - k_cz
